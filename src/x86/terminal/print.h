...
namespace funos
{
    namespace console
    {
        ...
        constexpr uint32_t VGA_MEMORY_ADDRESS = 0xB8000;
        constexpr uint8_t VGA_WIDTH = 80;
        constexpr uint8_t VGA_HEIGHT = 25;

        constexpr uint8_t VGA_COLOR_MASK = 0x0F;
        constexpr uint8_t VGA_BACKCOLOR_OFFSET = 4;
        constexpr uint8_t VGA_COLOR_ATTRIBUTE_SHIFT = 8;

        constexpr char CHAR_NEWLINE = '\n';

        constexpr uint32_t INT_SIGN_BIT_MASK = 0x80000000;
         ...
         */
        int setCursorPosition(unsigned x, unsigned y);
