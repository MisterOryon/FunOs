 ...
 * TAKEN: Indicates that the block is currently allocated and not available.
 * IS_FIRST: Marks the first block in a multi-block allocation.
 * HAS_NEXT: Indicates that this block is part of a multi-block allocation and continues to the next block.
 */
#define HEAP_BLOCK_TABLE_ENTRY_FLAG_TAKEN 0x01
#define HEAP_BLOCK_TABLE_ENTRY_FLAG_IS_FIRST 0x40
#define HEAP_BLOCK_TABLE_ENTRY_FLAG_HAS_NEXT  0x80
...
struct heap_table
{
    heap_block_table_entry_t* entries; // Array of entries, one for each block in the heap.
...
struct heap
{
    struct heap_table* table; // Pointer to the metadata table that tracks block allocation.
